...
class KnnLocationDateBlock(AbstractBaseBlock):
    def __init__(self, agg, n_neighbors, time_extend):
        ...
        self.col_name = f'knn_location_{self.agg}_{self.n_neighbors}_{self.time_extend}'
...
class PCAFeatureBlock(AbstractBaseBlock):
    ...
    def fit(self, input_df, y):
        output_df = input_df.select_dtypes([int, float]).drop('fold', axis=1)
        output_df = output_df.fillna(output_df.mean())

        self.num_ss.fit(output_df)
        self.pca.fit(output_df)

        return self.transform(output_df)
    
    def transform(self, input_df: pd.DataFrame) -> pd.DataFrame:
        output_df = input_df.select_dtypes([int, float])
        output_df = output_df.fillna(output_df.mean())

        output_col_names = [f'pca_{i}' for i in range(self.n_components)]

        output_df[output_df.columns] = self.num_ss.transform(output_df)
        output_df[output_col_names] = self.pca.transform(output_df)

        return output_df[output_col_names]
...
def create_feature(cfg, df_train, df_test):
    ...
    second_feature_blocks = [
        *[KnnLocationDateBlock(agg, n, t) for agg in ['mean', 'std'] for n in [5, 10] for t in [10]],
        ...
        *[TargetEncodingBlock(['month', 'Country', 'dayofweek', 'is_holiday'], df_train['fold'], agg) for agg in ['mean', 'std']],
        ...
        # *[AggBlock(group, ['co_mid', 'o3_mid', 'so2_mid', 'no2_mid']) for group in ['month', 'Country']],
    ]
    ...
    third_feature_blocks = [
        PCAFeatureBlock(10),
    ]
    df_train_feat = pd.concat([df_train_feat, run_blocks(df_train_feat, third_feature_blocks, y)], axis=1)
    df_test_feat = pd.concat([df_test_feat, run_blocks(df_test_feat, third_feature_blocks, test=True)], axis=1)
...
if __name__ == '__main__':
    df_train, _, _, df_test, _ = get_data()