...
class InteractionBlock(AbstractBaseBlock):
    def __init__(self, cols) -> None:
        self.cols = cols
    
    def transform(self, input_df: pd.DataFrame) -> pd.DataFrame:
        output_df = input_df[self.cols]
        for col1, col2 in itertools.combinations(self.cols, 2):
            # output_df[f"{col1}x{col2}"] = output_df[col1] * output_df[col2]
            output_df[f"{col1}/{col2}"] = output_df[col1] / output_df[col2]

        return output_df.drop(self.cols, axis=1)
...
def create_feature(df_train, df_test):
    ...
    first_feature_blocks = [
        ...
        InteractionBlock([pre+'_mid' for pre in ['co', 'o3', 'so2', 'no2']])
    ]
