...
class CategoricalBlock(AbstractBaseBlock):
    ...
    def transform(self, input_df):
        ...
        label_df[self.col] = np.vectorize(lambda x: self.label_dict_.get(x, self.label_other_))\
                                                        (input_df[self.col].values)
        if self.is_label:
            out_df = pd.concat([out_df, label_df], axis=1)
            
        if self.is_dummy:
            label_df[self.col] = label_df[f'{self.col}_label_enc'].astype(object)
...
def create_feature(df_train, df_test):
    ...
    first_feature_blocks = [
        ...
        *[CategoricalBlock(col) for col in ['Country', 'City']],
