...
def main():
    if cfg['debug']:
        cfg['n_splits'] = 2
        cfg['xgb_params']['n_estimators'] = 10
        cfg['name'] = 'debug'
    ...
    for i in range(cv.nunique()):
        ...
        with mlflow.start_run(experiment_id=cfg['experiment_id'], nested=True, ):
            train_dataset = xgb.DMatrix(X_train.loc[trn_idx], label=y_train.loc[trn_idx], enable_categorical=True)
            valid_dataset = xgb.DMatrix(X_train.loc[val_idx], label=y_train.loc[val_idx], enable_categorical=True)

            watchlist = [(train_dataset, 'train'), (valid_dataset, 'eval')]
            model = xgb.train(
                cfg['xgb_params'],
                train_dataset,
                num_round,
            ...
            joblib.dump(model, os.path.join(save_path, f'xgb_{i}.pkl'))
            models.append(model)
            oof.iloc[val_idx] = model.predict(valid_dataset)
    ...
    submit['pm25_mid'] = 0
    test_dataset = xgb.DMatrix(X_test, enable_categorical=True)
    for model in models:
        submit['pm25_mid'] += model.predict(test_dataset)
