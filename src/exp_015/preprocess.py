...
class KnnLocationDateBlock(AbstractBaseBlock):
    def __init__(self, agg, n_neighbors=10):
        self.n_neighbors=n_neighbors
        self.test_knn = None
        self.agg = agg
        self.num_ss = StandardScaler()
        self.col_name = f'knn_location_{self.agg}_{self.n_neighbors}'
    ...
    def fit(self, input_df, y):
        ...
        output_df['date_label'] = output_df['date_label'] * 5
        output_df['y'] = y
    ...
    def transform(self, input_df: pd.DataFrame) -> pd.DataFrame:
        ...
        output_df['date_label'] = output_df['date_label'] * 10
...
def create_feature(df_train, df_test):
    ...
    second_feature_blocks = [
        *[KnnLocationDateBlock(agg, n) for agg in ['mean', 'std'] for n in [5, 10]],
