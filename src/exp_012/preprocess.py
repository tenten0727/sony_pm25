...
class DatetimeBlock(AbstractBaseBlock):
    def transform(self, input_df: pd.DataFrame) -> pd.DataFrame:
        ...
        output_df[['year', 'month', 'day']] = output_df[['year', 'month', 'day']].astype('category')
...
class AggBlock(AbstractBaseBlock):
    def __init__(self, group, cols, aggs=[np.mean, np.max, np.min, np.std]):
        self.group = group
        self.cols = cols
        self.aggs = aggs
        self.test_agg = None
...
class NearbyCitiesFeatureBlock(AbstractBaseBlock):
    def __init__(self) -> None:
        self.rename = {i:f'{i}_nearby_City' for i in range(5)}
        self.nearby_cities_train = pd.read_pickle(SAVE_PATH+'/df_train_nearby_cities.pkl').head(5).T.reset_index().rename({'index':'City', **self.rename}, axis=1).astype('category')
        self.nearby_cities_test = pd.read_pickle(SAVE_PATH+'/df_test_nearby_cities.pkl').head(5).T.reset_index().rename({'index':'City', **self.rename}, axis=1).astype('category')

        self.df_lag_feature = None

    def fit(self, input_df, y):
        output_df = input_df[['date', 'City']]
        output_df = pd.merge(output_df, self.nearby_cities_train, how='left', on='City')

        return output_df[self.rename.values()]
    
    def transform(self, input_df: pd.DataFrame) -> pd.DataFrame:
        output_df = input_df[['date', 'City']]
        output_df = pd.merge(output_df, self.nearby_cities_test, how='left', on='City')
...
def create_feature(df_train, df_test):
    ...
    corr_top = ['no2_mid', 'co_mid', 'no2_max', 'no2_min', 'co_max']
    ...
    second_feature_blocks = [
        ...
        *[TargetEncodingBlock(['month', 'Country', 'dayofweek', 'is_holiday'], df_train['fold'], agg) for agg in ['mean', 'std']],
        ...
        # *[AggBlock(group, corr_top) for group in [ 'month']],
