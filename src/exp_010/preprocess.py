...
class LagLeadFeatureBlock(AbstractBaseBlock):
    def __init__(self, groups, cols, lag=1):
        self.groups = groups
        self.lag = lag
        self.cols = cols
        self.df_lag_feature = None
    ...
    def fit(self, input_df, y):
        self.df_lag_feature = input_df[self.groups+self.cols]
        self.df_lag_feature = self.df_lag_feature.groupby(self.groups)[self.cols].agg(self.agg).reset_index()
    ...
    def transform(self, input_df: pd.DataFrame) -> pd.DataFrame:
        ...
        output_cols = []
        for a in self.agg:
            output_cols.extend([col for col in output_df.columns if a in col])
        output_df = output_df[output_cols]
        output_df.columns = [f"lag{self.lag}{'_'.join(self.groups)}_{'_'.join(col)}" for col in output_df.columns]
...
def create_feature(df_train, df_test):
    ...
    second_feature_blocks = [
        *[YLagLeadBlock(['Country', 'date'], lag=i) for i in [-10, -7, -3, -2, -1, 1, 2, 3, 7, 10]],
        # *[LagLeadFeatureBlock(['Country', 'date'], ['co_mid', 'o3_mid', 'so2_mid', 'no2_mid'], lag=i) for i in [-3, -2, -1, 1, 2, 3]],
