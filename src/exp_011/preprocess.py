...
class NearbyCitiesFeatureBlock(AbstractBaseBlock):
    def __init__(self) -> None:
        self.nearby_cities_train = pd.read_pickle(SAVE_PATH+'/df_train_nearby_cities_TE.pkl').head(5).T.reset_index().rename({'index':'City'}, axis=1)
        self.nearby_cities_test = pd.read_pickle(SAVE_PATH+'/df_test_nearby_cities_TE.pkl').head(5).T.reset_index().rename({'index':'City'}, axis=1)

    def fit(self, input_df, y):
        output_df = input_df[['City']]
        output_df = pd.merge(output_df, self.nearby_cities_train, how='left', on='City').drop('City', axis=1)
        output_df['mean_nearby_cities_TE'] = output_df.mean(axis=1)

        # output_df.columns = ['city_nearby'+str(i) for i in range(5)]

        return output_df[['mean_nearby_cities_TE']]
    
    def transform(self, input_df: pd.DataFrame) -> pd.DataFrame:
        output_df = input_df[['City']]
        output_df = pd.merge(output_df, self.nearby_cities_test, how='left', on='City').drop('City', axis=1)
        output_df['mean_nearby_cities_TE'] = output_df.mean(axis=1)
