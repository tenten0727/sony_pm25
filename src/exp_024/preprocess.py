...
class LatLonCategoryBlock(AbstractBaseBlock):
    def __init__(self, round=60) -> None:
        self.round = round
    
    def transform(self, input_df: pd.DataFrame) -> pd.DataFrame:
        output_df = input_df[['lon', 'lat']]
        output_df['lon_round'] = output_df['lon'] // self.round
        output_df['lat_round'] = output_df['lat'] // self.round
        output_df['location_category'] = ((output_df['lon_round']).astype(str)+ '-' + output_df['lat_round'].astype(str)).astype('category')

        return output_df[['location_category']]
...
def create_feature(df_train, df_test):
    ...
    second_feature_blocks = [
        ...
        *[YLagLeadBlock(['Country', 'date', 'kmeans_cluster'], lag=i) for i in [-10, -7, -3, -2, -1, 1, 2, 3, 7, 10]],
        *[TargetEncodingBlock(['month', 'Country', 'dayofweek', 'is_holiday', 'kmeans_cluster'], df_train['fold'], agg) for agg in ['mean', 'std']],
        CountEncodingBlock(['year', 'month', 'day', 'Country', 'dayofweek', 'kmeans_cluster']),
