...
class UmapFeatureBlock(AbstractBaseBlock):
    ...
    def fit(self, input_df: pd.DataFrame, y=None):
        output_df = input_df.select_dtypes(include=[int, float]).drop('fold', axis=1)
    ...
    def transform(self, input_df: pd.DataFrame) -> pd.DataFrame:
        output_df = input_df.select_dtypes(include=[int, float])
        output_df = output_df.fillna(output_df.mean())

        return pd.DataFrame(self.pipe.transform(output_df), columns=['umap_x', 'umap_y'])
